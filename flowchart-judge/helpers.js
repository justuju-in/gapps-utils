...
function saveCodeToDrive(timestamp, email, problemId, code) {
  ...
  var filename = `${safeTimestamp}_${safeEmail}_${problemId}${getConfig().generatedCodeFileExtension}`;
  ...
  try {
    ...
    logEvent(`saveCodeToDrive: File saved. filename=${filename}, url=${file.getUrl()}`, getConfig().logLevelDebug);
    return file.getUrl();
  } catch (e) {
...
function getCodeFromDriveUrl(fileUrl) {
  ...
  if (!fileIdMatch){
    logEvent('getCodeFromDriveUrl: Invalid file URL: ' + fileUrl, getConfig().logLevelError);
    return null;
  }
  try {
    var file = DriveApp.getFileById(fileIdMatch[0]);
    var code = file.getBlob().getDataAsString();
    logEvent('getCodeFromDriveUrl: Read code from fileId=' + fileIdMatch[0], getConfig().logLevelDebug);
    return code;
  } catch (e) {
...
function getColumnIndexByName(sheet, name) {
  ...
  if (idx === 0) {
    logEvent('getColumnIndexByName: Column not found: ' + name, getConfig().logLevelError);
...
function getFileIdFromUrl(url) {
  ...
  if (matches) {
    logEvent('getFileIdFromUrl: Extracted fileId=' + matches[0], getConfig().logLevelDebug);
    return matches[0];
  } else {
    logEvent('getFileIdFromUrl: No fileId found in url: ' + url, getConfig().logLevelError);
    return null;
  }
}
...
function getProblemIdFromMeta(problemCode) {
  ...
  if (!metaSheet) {
    logEvent('getProblemIdFromMeta: Meta sheet not found', getConfig().logLevelError);
    return null;
  }
  var data = metaSheet.getDataRange().getValues();
  for (var i = 1; i < data.length; i++) { // skip header
    if (data[i][0] === problemCode) {
      logEvent('getProblemIdFromMeta: Found problemId=' + data[i][1] + ' for problemCode=' + problemCode, getConfig().logLevelDebug);
      return data[i][1]; // Column B: Problem id
    }
  }
  logEvent('getProblemIdFromMeta: Problem code ' + problemCode + ' not found in Meta sheet', getConfig().logLevelError);
  return null;
}
